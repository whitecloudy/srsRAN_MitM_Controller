...
namespace srsran
{
  namespace nas_5g
  {
    ...
    const char* mobile_identity_5gs_t::suci_s::supi_format_type_::to_string() const
    {
      switch (value)
      {
        case suci_s::supi_format_type_::imsi:
          return "IMSI";
        case suci_s::supi_format_type_::network_specific_identifier:
          return "Network specific identifier";
        case suci_s::supi_format_type_::gci:
          return "GCI";
        case suci_s::supi_format_type_::gli:
          return "GLI";
        default:
          return "Invalid Choice";
      }
    }
    const char* mobile_identity_5gs_t::suci_s::protection_scheme_id_type_::to_string() const
    {
      switch (value)
      {
        case suci_s::protection_scheme_id_type_::null_scheme:
          return "Null scheme";
        case suci_s::protection_scheme_id_type_::ecies_scheme_profile_a:
          return "ECIES scheme profile A";
        case suci_s::protection_scheme_id_type_::ecies_scheme_profile_b:
    ...
    void mobile_identity_5gs_t::suci_s::to_json(json_writer &j)
    {
      j.write_str("SUPI format", supi_format.to_string());
      uint32_t mnc_v = 0;
      uint32_t mcc_v = 0;
      for (int i = 0; i < 3; i++)
      {
      ...
      uint32_t routing_indicator_v = 0;
      for (int i = 0; i < 4; i++)
      {
        routing_indicator_v <<= 8;
        routing_indicator_v += routing_indicator[i];
      }

      j.write_int("Routing indicator", routing_indicator_v);

      j.write_str("Protection scheme Id", protection_scheme_id.to_string());
      j.write_int("Home network public key identifier", home_network_public_key_identifier);

      j.write_str("Scheme output", octstring_to_string(scheme_output.data(), scheme_output.size()));
    }
    ...
    void mobile_identity_5gs_t::guti_5g_s::to_json(json_writer &j)
    {
      uint32_t mnc_v = 0;
      uint32_t mcc_v = 0;
      for (int i = 0; i < 3; i++)
      ...
      j.write_int("AMF Region ID", amf_region_id);
      j.write_int("AMF Set ID", amf_set_id);
      j.write_int("AMF Pointer", amf_pointer);
      j.write_int("5G-TMSI", tmsi_5g);
    }
    ...
    void mobile_identity_5gs_t::imei_s::to_json(json_writer &j)
    {
      j.write_str("IMEI", octstring_to_string(imei.data(), imei.size()));
    }
    ...
    void mobile_identity_5gs_t::s_tmsi_5g_s::to_json(json_writer &j)
    {
      j.write_int("AMF Set ID", amf_set_id);
      j.write_int("AMF Pointer", amf_pointer);
      j.write_int("5G-TMSI", tmsi_5g);
    ...
    void mobile_identity_5gs_t::imeisv_s::to_json(json_writer &j)
    {
      j.write_str("IMEISV", octstring_to_string(imeisv.data(), imeisv.size()));
    }
    ...
    void mobile_identity_5gs_t::mac_address_s::to_json(json_writer &j)
    {
    ...
    SRSASN_CODE mobile_identity_5gs_t::eui_64_s::unpack(asn1::cbit_ref &bref, uint8_t tmp, uint32_t length)
    {
      return SRSASN_SUCCESS;
        ...
        void mobile_identity_5gs_t::eui_64_s::to_json(json_writer &j)
    {
