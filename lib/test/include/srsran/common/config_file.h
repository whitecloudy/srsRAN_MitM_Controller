...
bool config_exists(std::string& filename, std::string default_name)
{
  ...
  if (conf.fail()) {
    ...
    snprintf(full_path, sizeof(full_path), "%s/.config/mitm_lib/%s", homedir, default_name.c_str());
    ...
    if (conf.fail()) {
      ...
      ZERO_OBJECT(full_path);
      snprintf(full_path, sizeof(full_path), "/etc/mitm_lib/%s", default_name.c_str());
