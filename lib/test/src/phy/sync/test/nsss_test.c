...
#include "mitm_lib/phy/sync/nsss.h"
...
int      sfn        = 0;
uint32_t n_id_ncell = SRSRAN_CELL_ID_UNKNOWN;
...
#define SFLEN (1 * SRSRAN_SF_LEN(SRSRAN_NBIOT_FFT_SIZE))
...
int main(int argc, char** argv)
{
  ...
  buffer = srsran_vec_cf_malloc(SFLEN * max_num_sf);
  if (!buffer) {
    perror("malloc");
    return ret;
  }
  ...
  fft_buffer = srsran_vec_cf_malloc(SFLEN * max_num_sf);
  if (!fft_buffer) {
    perror("malloc");
    goto exit;
  }
  ...
  if (srsran_ofdm_tx_init(&ifft, SRSRAN_CP_NORM, buffer, fft_buffer, NOF_PRB)) {
  ...
  if (input_file_name != NULL) {
    ...
    // offset file
    int file_offset = 0;
    srsran_filesource_read(&fsrc, buffer, file_offset);

    // now read
    while (num_sf < max_num_sf) {
      int n = srsran_filesource_read(&fsrc, &fft_buffer[num_sf * SFLEN], SFLEN);
      if (n < 0) {
  ...
  if (srsran_nsss_synch_init(&syncobj, num_sf * SFLEN, fft_size)) {
  ...
  if (!input_file_name) {
    ...
    cf_t nsss_signals[SRSRAN_NSSS_TOT_LEN] = {};
    srsran_nsss_generate(nsss_signals, n_id_ncell == SRSRAN_CELL_ID_UNKNOWN ? 0 : n_id_ncell);

#if DUMP_SIGNALS
    srsran_vec_save_file("nsss_signal_freq.bin", nsss_signals, SRSRAN_NSSS_LEN * sizeof(cf_t));
#endif

    srsran_nsss_put_subframe(
        &syncobj, nsss_signals, buffer, sfn, SRSRAN_NBIOT_DEFAULT_NUM_PRB_BASECELL, SRSRAN_NBIOT_DEFAULT_PRB_OFFSET);
  ...
  uint32_t n_id_ncell_detected = SRSRAN_CELL_ID_UNKNOWN;
  uint32_t sfn_partial         = 0;
  srsran_nsss_sync_find(&syncobj, fft_buffer, &peak_value, &n_id_ncell_detected, &sfn_partial);
  printf("NSSS with peak=%f, n_id_ncell: %d, partial SFN: %x\n", peak_value, n_id_ncell_detected, sfn_partial);

  if (n_id_ncell_detected == (n_id_ncell == SRSRAN_CELL_ID_UNKNOWN ? 0 : n_id_ncell)) {
    printf("Ok\n");
    ret = SRSRAN_SUCCESS;
  } else {
    printf("Failed\n");
...
#define MAX_FNAME_LEN 40
  ...
  srsran_vec_save_file(fname, fft_buffer, num_sf * SFLEN * sizeof(cf_t));
  srsran_vec_save_file("nsss_corr_seq_time.bin",
                       syncobj.nsss_signal_time[n_id_ncell == SRSRAN_CELL_ID_UNKNOWN ? 0 : n_id_ncell],
                       SRSRAN_NSSS_CORR_FILTER_LEN * sizeof(cf_t));
  if (n_id_ncell_detected != SRSRAN_CELL_ID_UNKNOWN) {
    // run correlation again with found cell to populate conv_output_abs
    srsran_nsss_sync_find(&syncobj, fft_buffer, &peak_value, &n_id_ncell_detected, &sfn_partial);
  }
  srsran_vec_save_file("nsss_find_conv_output_abs.bin",
                       syncobj.conv_output_abs,
                       (SRSRAN_NSSS_CORR_FILTER_LEN + num_sf * SFLEN) * sizeof(float));
  ...
  // cleanup
exit:
  srsran_nsss_synch_free(&syncobj);
