...
#include "mitm_lib/phy/fec/ldpc/ldpc_common.h"
#include "mitm_lib/phy/fec/ldpc/ldpc_decoder.h"
...
int main(int argc, char** argv)
{
  ...
  symbols       = srsran_vec_i16_malloc(finalN * NOF_MESSAGES);
  ...
  for (j = 0; j < NOF_MESSAGES; j++) {
    printf("  codeword %d\n", j);
    srsran_ldpc_decoder_decode_s(&decoder, symbols + j * finalN, messages_sim + j * finalK, finalN);
  }
