...
//  pp->f        = srsran_vec_function_f_ccc;
//  pp->g        = srsran_vec_function_g_bccc;
//  pp->xor      = srsran_vec_xor_bbb;
//  pp->hard_bit = srsran_vec_hard_bit_cc;

#include "polar_decoder_ssc_c.h"
#include "../utils_avx2.h"
#include "polar_decoder_vector.h"
#include "mitm_lib/phy/fec/polar/polar_encoder.h"
...
void* create_polar_decoder_ssc_c(const uint8_t nMax)
{
  ...
  SRSRAN_MEM_ZERO(pp, struct pSSC_c, 1);
  ...
  if ((pp->enc = SRSRAN_MEM_ALLOC(srsran_polar_encoder_t, 1)) == NULL) {
  ...
  if ((pp->param = SRSRAN_MEM_ALLOC(struct Params, 1)) == NULL) {
    delete_polar_decoder_ssc_c(pp);
    return NULL;
  }
  ...
  pp->llr0[0] = srsran_vec_i8_malloc(llr_all_stages); // 32*8=256
  // allocate memory to the polar decoder instance
  if (pp->llr0[0] == NULL) {
    free(pp->est_bit);
    free(pp->state);
