...
const uint32_t f2_list[SRSRAN_NOF_TC_CB_SIZES] = {
...
int srsran_tc_interl_LTE_gen(srsran_tc_interl_t* h, uint32_t long_cb)
{
  return srsran_tc_interl_LTE_gen_interl(h, long_cb, 1);
}

#define deinter(x, win) ((x % (long_cb / win)) * (win) + x / (long_cb / win))
#define inter(x, win) ((x % win) * (long_cb / win) + x / win)

int srsran_tc_interl_LTE_gen_interl(srsran_tc_interl_t* h, uint32_t long_cb, uint32_t interl_win)
{
  ...
  cb_table_idx = srsran_cbsegm_cbindex(long_cb);
  if (cb_table_idx == -1) {
    ERROR("Can't find long_cb=%d in valid TC CB table", long_cb);
    return -1;
  }
  ...
  if (interl_win != 1) {
    uint16_t* f = srsran_vec_u16_malloc(long_cb);
    uint16_t* r = srsran_vec_u16_malloc(long_cb);
